...
from maninsist_game_custom_wrapper_cnn import PlayEnv
...
def make_env(seed=0):
    def _init():
        env = PlayEnv(seed=seed)
        env = ActionMasker(env, PlayEnv.get_action_mask)
        env = Monitor(env)
        env.seed(seed)
        return env
    return _init
...
def main():
    ...
    # ??? checkpoint_interval (好像和游戏无关，是保存训练过程用来看tensorboard用的)
    checkpoint_interval = 15625 # checkpoint_interval * num_envs = total_steps_per_checkpoint

    # ??? checkpoint_callback (好像和游戏无关，是保存训练过程用来看tensorboard用的)
